...
RegisterCommand('startfueling', function()
	...
	if nearestPump then
		...
		if petrolCan and moneyAmount >= fuel.getVipPrice(config.petrolCan.refillPrice) then
		...
		if not vehicleInRange then
			...
			if moneyAmount >= fuel.getVipPrice(config.petrolCan.price) then
				return fuel.getPetrolCan(nearestPump)
			end

			return lib.notify({ type = 'error', description = locale('petrolcan_cannot_afford') })
		elseif moneyAmount >= fuel.getVipPrice(config.priceTick) then
