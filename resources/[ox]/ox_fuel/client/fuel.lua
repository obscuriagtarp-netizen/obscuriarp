...
function fuel.getVipPrice(amount)
	amount = math.max(0, tonumber(amount) or 0)
	local vip = LocalPlayer.state.obVip
	local percent = vip and vip.benefits and tonumber(vip.benefits.fuelDiscount) or 0
	percent = math.min(100, math.max(0, percent))
	return math.max(0, math.floor(amount * (1 - percent / 100) + 0.5))
end
...
function fuel.startFueling(vehicle, isPump)
	...
	if isPump then
		...
		if fuel.getVipPrice(config.priceTick) > moneyAmount then
			return lib.notify({
				type = 'error',
				description = locale('not_enough_money', fuel.getVipPrice(config.priceTick))
			})
		end
	...
	while state.isFueling do
		if isPump then
			...
			if fuel.getVipPrice(price + config.priceTick) >= moneyAmount and lib.progressActive() then
				lib.cancelProgress()
			end
