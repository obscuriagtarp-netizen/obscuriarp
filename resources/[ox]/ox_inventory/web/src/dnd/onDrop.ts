import { canOccupyPlayerSlot, canStack, findAvailableSlot, getTargetInventory, isSlotWithItem } from '../helpers';
...
export const onDrop = (source: DragSource, target?: DropTarget) => {
  ...
  const targetData = targetSlot.name ? Items[targetSlot.name] : undefined;

  if (!canOccupyPlayerSlot(targetInventory.type, targetSlot.slot, sourceData)) return;
  if (!canOccupyPlayerSlot(sourceInventory.type, sourceSlot.slot, targetData)) return;
