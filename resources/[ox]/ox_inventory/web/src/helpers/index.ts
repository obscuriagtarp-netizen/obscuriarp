...
export const equipmentSlotTypes: Record<number, NonNullable<ItemData['equipment']>> = {
  6: 'amulet',
  7: 'necklace',
  8: 'belt',
  9: 'ring',
...
export const canOccupyPlayerSlot = (inventoryType: string, slot: number, item?: ItemData) => {
  if (inventoryType !== InventoryType.PLAYER) return true;

  const equipmentType = equipmentSlotTypes[slot];

  return !equipmentType || !item || item.equipment === equipmentType;
};

export const findAvailableSlot = (item: Slot, data: ItemData, items: Slot[], inventoryType?: string) => {
  const availableItems =
    inventoryType === InventoryType.PLAYER ? items.filter((target) => !equipmentSlotTypes[target.slot]) : items;

  if (!data.stack) return availableItems.find((target) => target.name === undefined);

  const stackableSlot = availableItems.find(
    (target) => target.name === item.name && isEqual(target.metadata, item.metadata)
  );

  return stackableSlot || availableItems.find((target) => target.name === undefined);
};
