...
function Utils.nearbyMarker(point)
    if point.markerImage then
        exports.ob_markers:Draw(point.markerImage, point.coords, {
    ...
    if point.isClosest and point.currentDistance < 1.2 then
        if not point.markerImage and not hasTextUi then
            hasTextUi = point
            lib.showTextUI(point.prompt.message, point.prompt.options)
        elseif point.markerImage and hasTextUi == point then
            hasTextUi = nil
            lib.hideTextUI()
        end
