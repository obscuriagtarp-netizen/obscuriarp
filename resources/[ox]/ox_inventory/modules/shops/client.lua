...
local shopTargetEnabled = shared.target and GetConvarInt('inventory:shopTarget', 0) == 1

local function markerForShop(shopType)
	return tostring(shopType):lower() == 'ammunation' and 'ammunation' or 'loja'
end

for shopType, shopData in pairs(lib.load('data.shops') or {} --[[@as table<string, OxShop>]]) do
	local useTarget = shared.target and (shopData.useTarget == true or (shopData.useTarget ~= false and shopTargetEnabled))
	local shop = {
		name = shopData.name,
		...
		elseif shop.locations then
			...
			for i = 1, #shop.locations do
				...
				shops[id] = lib.points.new(coords, 16, {
					...
					invId = i,
					type = type,
                    markerImage = markerForShop(type),
