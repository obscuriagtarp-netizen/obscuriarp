...
local function requiredEquipmentType(inv, slot)
	return inv and inv.type == 'player' and equipmentSlots[tonumber(slot)] or nil
end

local function canOccupyEquipmentSlot(inv, slot, item)
	local requiredType = requiredEquipmentType(inv, slot)

	return not requiredType or (item and item.equipment == requiredType)
end

function Inventory.NormalizeEquipmentSlots(items, slotCount)
	local changed = false
	local blocked = false
	...
	for slot, equipmentType in pairs(equipmentSlots) do
		...
		if slotData and (not item or item.equipment ~= equipmentType) then
			local targetSlot

			for index = 10, slotCount do
			...
			if targetSlot then
				slotData.slot = targetSlot
				items[targetSlot] = slotData
				items[slot] = nil
				changed = true
			else
				blocked = true
...
function Inventory.GetEquippedItems(inv)
	local inventory = Inventory(inv)

	if not inventory or inventory.type ~= 'player' then return {} end
...
exports('GetEquippedItems', Inventory.GetEquippedItems)

local function emitEquipmentChanged(inv)
	if inv and inv.player then
		TriggerEvent('ox_inventory:server:equipmentChanged', inv.id, Inventory.GetEquippedItems(inv))
	end
end
...
function Inventory.AddItem(inv, item, count, metadata, slot, cb)
	...
	if slot then
		if not canOccupyEquipmentSlot(inv, slot, item) then
	...
	if not toSlot then
		...
		for i = 1, inv.slots do
			if not requiredEquipmentType(inv, i) then
...
lib.callback.register('ox_inventory:swapItems', function(source, data)
	...
	if toInventory and (data.toType == 'newdrop' or fromInventory ~= toInventory or data.fromSlot ~= data.toSlot) then
		...
		if not canOccupyEquipmentSlot(toInventory, data.toSlot, Items(fromData.name)) then
			return false
		end

		if toData and not canOccupyEquipmentSlot(fromInventory, data.fromSlot, Items(toData.name)) then
			return false
		end
		...
		if fromData then
			...
			if requiredEquipmentType(fromInventory, data.fromSlot) or requiredEquipmentType(toInventory, data.toSlot) then
				emitEquipmentChanged(fromInventory)

				if toInventory ~= fromInventory then
					emitEquipmentChanged(toInventory)
				end
			end
...
function Inventory.GetEmptySlot(inv)
	...
	for i = 1, inventory.slots do
		if not requiredEquipmentType(inventory, i) and not items[i] then
			return i
		end
	end
end
...
function Inventory.GetSlotForItem(inv, itemName, metadata)
	...
	for i = 1, inventory.slots do
		if not requiredEquipmentType(inventory, i) then
			local slotData = items[i]
