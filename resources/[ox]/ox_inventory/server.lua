...
function server.setPlayerInventory(player, data)
	...
	local equipmentMigrated, equipmentMigrationBlocked = Inventory.NormalizeEquipmentSlots(inventory, shared.playerslots)
	...
	if inv then
		if equipmentMigrated then inv.changed = true end

		if equipmentMigrationBlocked then
			print(('[ox_inventory] Nao foi possivel liberar todos os slots de equipamento de %s; o inventario esta cheio.'):format(player.identifier))
		end
...
local function openInventory(source, invType, data, ignoreSecurityChecks)
	...
	return {
		...
		owner = left.owner,
		class = server.getPlayerClass and server.getPlayerClass(left.id) or left.player and left.player.class
	}, right and {
		id = right.id,
		label = right.player and '' or right.label,
		ownerLabel = right.label,
...
local function hasWitchPouch(inventory)
...
local function canOpenWitchPouch(source, data)
	if type(data) ~= 'table' or data.id ~= 'witch_pouch' then return true end

	local playerInventory = Inventory(source)
	local playerClass = server.getPlayerClass and server.getPlayerClass(source) or playerInventory and playerInventory.player.class
	playerClass = string.lower(tostring(playerClass or ''))
	if not playerInventory or playerClass ~= 'bruxa' then return false end

	local requestedOwner = data.owner or playerInventory.owner
	data.owner = requestedOwner

	if requestedOwner == playerInventory.owner then
		return true
	end

	local targetId = tonumber(data.targetId)
	local targetInventory = targetId and Inventory(targetId)
	if not targetInventory or targetInventory.owner ~= requestedOwner or not hasWitchPouch(targetInventory) then
		return false
	end

	local playerPed = playerInventory.player.ped
	local targetPed = targetInventory.player and targetInventory.player.ped
	if not playerPed or not targetPed then return false end
...
lib.callback.register('ox_inventory:openInventory', function(source, invType, data)
	if invType == 'stash' and type(data) == 'table' and data.id == 'witch_pouch' then
		if not canOpenWitchPouch(source, data) then return end
	end
