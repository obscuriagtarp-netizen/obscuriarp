...
local function hasWitchPouch(items)
	if not items then return false end

	for _, item in pairs(items) do
		if item and item.name == 'witch_pouch' and (tonumber(item.count) or 0) > 0 then
			return true
		end
	end
...
local function getWitchPouchContext(left, right)
	local className = string.lower(tostring(left and left.class or PlayerData.class or ''))
	if className ~= 'bruxa' then
		return { visible = false, present = false }
	end

	local rightId = tostring(right and right.id or '')
	local pouchOpened = right
		and right.type == 'stash'
		and (rightId == 'witch_pouch' or rightId:sub(1, 12) == 'witch_pouch:')
	...
	if pouchOpened then
		return {
			...
			opened = true,
			owner = right.owner,
			label = right.label or 'Bolsa Arcana'
		}
	end

	local inspected = right and (right.type == 'otherplayer' or right.type == 'inspect') or false
	local targetInventory = inspected and right or left

	return {
		visible = true,
		-- A bolsa pessoal faz parte da interface da bruxa. Durante uma revista,
		-- a bolsa so aparece como disponivel se o alvo realmente possuir o item.
		present = not inspected or hasWitchPouch(targetInventory and targetInventory.items),
		opened = false,
		inspected = inspected,
		owner = targetInventory and targetInventory.owner or PlayerData.identifier,
		targetId = inspected and tonumber(targetInventory.id) or nil,
		label = inspected and (targetInventory.ownerLabel or 'Pessoa revistada') or (left and left.label or PlayerData.name or 'Bruxa')
	}
end
...
function client.openInventory(inv, data)
	...
	PlayerData.class = left.class or PlayerData.class
    ...
    SendNUIMessage({
        ...
        data = {
			...
			witchPouch = getWitchPouchContext(left, currentInventory)
        }
    })
...
local function getEquippedItems()
	...
	for slot, equipmentType in pairs(equipmentSlots) do
		local slotData = inventory[slot]
		local item = slotData and Items[slotData.name]

		if slotData and item and item.equipment == equipmentType then
			local data = table.clone(slotData)
			data.equipment = equipmentType
			data.attributes = item.attributes or {}
			equipped[equipmentType] = data
		end
	end
...
local function updateInventory(data, weight)
	...
	TriggerEvent('ox_inventory:equipmentChanged', getEquippedItems())
end
...
local function openWitchPouch(data)
	data = type(data) == 'table' and data or {}
	local owner = data.owner or PlayerData.identifier
	...
	if not invOpen then
		return client.openInventory('stash', payload) == true
	end

	local left, right, accessError = lib.callback.await('ox_inventory:openInventory', false, 'stash', payload)
	if accessError or not left or not right then return false end
...
RegisterNetEvent('ox_inventory:openWitchPouch', function()
	CreateThread(function()
		openWitchPouch({ owner = PlayerData.identifier })
	end)
end)

RegisterNUICallback('openWitchPouch', function(data, cb)
	cb(openWitchPouch(data))
end)
