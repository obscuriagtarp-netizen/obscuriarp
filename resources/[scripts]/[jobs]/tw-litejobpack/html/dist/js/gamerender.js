const config = {
  ...
  maxBlurSize: 8,  // 4K flicker fix: reduced from 20 → 8 (sample count: 1681 → 289 per pixel)
};
...
function createGameView(canvas, resolutionScale = config.resolutionScale) {
  ...
  function render() {
    // Re-query elements each frame to catch Vue's dynamic v-for elements
    // The Obscuria job UI uses solid surfaces. Keep the renderer available for
    // future opt-in glass elements without applying blur to the job interface.
    const glassElements = document.querySelectorAll('[data-obscuria-glass]');
    ...
    glassElements.forEach(element => {
      ...
      // Skip hidden or zero-size elements (v-show="false", opacity:0, v-if etc.)
      ...
      const borderRadius = parseFloat(borderRadiusStr);
      const isCircle = borderRadiusStr.includes('%') && borderRadius >= 50;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Always create canvas - elements will be queried dynamically in render loop
  ...
  // 4K flicker fix: drop render scale on high-DPR displays so the GL pass doesn't saturate the GPU
  const dpr = window.devicePixelRatio || 1;
  const adaptiveScale = dpr >= 2 ? 0.33 : (dpr >= 1.5 ? 0.4 : config.resolutionScale);
  const gameView = createGameView(canvas, adaptiveScale);
