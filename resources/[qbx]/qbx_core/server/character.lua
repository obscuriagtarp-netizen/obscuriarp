...
local function getAllowedAmountOfCharacters(source, license2, license)
    local configured = config.characters.playersNumberOfCharacters[license2]
        or license and config.characters.playersNumberOfCharacters[license]
        or config.characters.defaultNumberOfCharacters
    local purchased = 0

    if GetResourceState('magicPauseObscuria') == 'started' then
        local ok, amount = pcall(function()
            return exports.magicPauseObscuria:GetExtraCharacterSlots(source, license)
        end)
        if ok then purchased = math.max(0, math.floor(tonumber(amount) or 0)) end
    end

    return math.max(1, math.floor(tonumber(configured) or 1)) + purchased
end
...
lib.callback.register('qbx_core:server:createCharacter', function(source, data)
    ...
    local license2, license = GetPlayerIdentifierByType(source, 'license2'), GetPlayerIdentifierByType(source, 'license')
    local characters = storage.fetchAllPlayerEntities(license2, license)
    local allowedCharacters = getAllowedAmountOfCharacters(source, license2, license)
    local requestedSlot = math.floor(tonumber(data.cid) or 0)

    if #characters >= allowedCharacters or requestedSlot < 1 or requestedSlot > allowedCharacters then return end

    for _, character in ipairs(characters) do
        local occupiedSlot = tonumber(character.charinfo and character.charinfo.cid)
        if occupiedSlot == requestedSlot then return end
    end
