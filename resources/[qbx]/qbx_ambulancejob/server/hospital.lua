...
local function medicalCost(identifier, amount)
	...
	return math.max(0, tonumber(finalAmount) or amount), math.max(0, tonumber(discount) or 0), math.max(0, tonumber(percent) or 0)
end
...
local function billPlayer(src)
	...
	if GetResourceState('ob_hospital') == 'started' then
		local ok, covered = pcall(function()
			return exports.ob_hospital:HasActivePlan(player.PlayerData.citizenid)
		end)
		if ok and covered then
			exports.qbx_core:Notify(src, 'Atendimento coberto pelo seu plano hospitalar.', 'success')
			return
		end
	end
	local amount, _, discountPercent = medicalCost(src, sharedConfig.checkInCost)
	player.Functions.RemoveMoney('bank', amount, 'respawned-at-hospital')
	config.depositSociety('ambulance', amount)
	TriggerClientEvent('hospital:client:SendBillEmail', src, amount)
	if discountPercent > 0 then
		exports.qbx_core:Notify(src, ('Desconto VIP de %s%% aplicado ao atendimento.'):format(discountPercent), 'success')
	end
end
...
local function wipeInventory(src)
	...
	if GetResourceState('ob_vip') == 'started' then
		local ok, items = pcall(function() return exports.ob_vip:HandleRespawn(src) end)
		if ok and type(items) == 'table' and #items > 0 then keep = items end
	end
