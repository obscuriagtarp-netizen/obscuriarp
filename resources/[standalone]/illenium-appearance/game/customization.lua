...
local function setCamera(key)
    if not isCameraInterpolating then
        ...
        local framingOffset = constants.CAMERA_FRAMING[currentCamera] or 0.0
        local reverseFactor = reverseCamera and -1 or 1

        if cameraHandle then
            local camCoords = GetOffsetFromEntityInWorldCoords(cache.ped, (coords.x + framingOffset) * reverseFactor, coords.y * reverseFactor, coords.z * reverseFactor)
            local camPoint = GetOffsetFromEntityInWorldCoords(cache.ped, (point.x + framingOffset) * reverseFactor, point.y, point.z)
...
function client.rotateCamera(direction)
    if not isCameraInterpolating then
        ...
        local camCoords = GetOffsetFromEntityInWorldCoords(
            cache.ped,
            (((coords.x + offset.x) * sideFactor) + framingOffset) * reverseFactor,
