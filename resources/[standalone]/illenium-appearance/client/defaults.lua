...
function SetCustomizationContext(config, contextName)
    local contexts = Config.AppearanceContexts or {}
    local context = contexts[contextName] or contexts.default or {}

    config.uiEyebrow = context.eyebrow or "Obscuria"
    config.uiTitle = context.title or "Personalização de Personagem"
    config.uiDescription = context.description or "Ajuste sua aparência antes de continuar."

    return config
end

function GetDefaultConfig()
    return SetCustomizationContext({
        ped = false,
        headBlend = false,
        faceFeatures = false,
        headOverlays = false,
