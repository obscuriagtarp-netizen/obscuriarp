...
local function ConvertToCollectionFormat(ped, props, components)
    ...
    for _, data in pairs(components or {}) do
        ...
        if componentId then
            ...
            local localDrawable = tonumber(GetPedCollectionLocalIndexFromDrawable(ped, componentId, drawable)) or -1
            local converted = {
            ...
            if type(collection) == "string" and localDrawable >= 0 then
                converted.collection = collection
                converted.drawable = localDrawable
            end

            result.components[#result.components + 1] = converted
        end
    end

    for _, data in pairs(props or {}) do
        ...
        if propId then
            ...
            local converted = {
                prop_id = propId,
                drawable = drawable,
                texture = drawable == -1 and -1 or texture
            }

            if drawable ~= -1 then
                local collection = GetPedCollectionNameFromProp(ped, propId, drawable)
                local localDrawable = tonumber(GetPedCollectionLocalIndexFromProp(ped, propId, drawable)) or -1
                if type(collection) == "string" and localDrawable >= 0 then
                    converted.collection = collection
                    converted.drawable = localDrawable
                end
            ...
            result.props[#result.props + 1] = converted
        end
    end
...
local function PrepareAppearanceForSave(ped, appearance)
    if type(appearance) ~= "table" then return appearance end

    local converted = ConvertToCollectionFormat(ped, appearance.props, appearance.components)
    appearance.props = converted.props
    appearance.components = converted.components
    return appearance
end

local function UsesCollectionFormat(appearance)
    if type(appearance) ~= "table" then return false end

    for _, component in pairs(appearance.components or {}) do
        if type(component) == "table" and component.collection ~= nil then
            return true
        end
    end
...
function InitAppearance()
    ...
    lib.callback("illenium-appearance:server:getAppearance", false, function(appearance)
        ...
        local needsMigration = not UsesCollectionFormat(appearance)
        client.setPlayerAppearance(appearance)
        if needsMigration then
            PrepareAppearanceForSave(PlayerPedId(), appearance)
...
local function getNewCharacterConfig()
    ...
    SetCustomizationContext(config, "character")
...
function InitializeCharacter(gender, onSubmit, onCancel)
    ...
    client.startPlayerCustomization(function(appearance)
        if (appearance) then
            PrepareAppearanceForSave(cache.ped, appearance)
            TriggerServerEvent("illenium-appearance:server:saveAppearance", appearance)
            if onSubmit then
                onSubmit()
            end
...
local function OpenClothingShop(isPedMenu)
    local config = GetDefaultConfig()
    SetCustomizationContext(config, isPedMenu and "complete" or "clothing")
...
RegisterNetEvent("illenium-appearance:client:saveOutfit", function()
    ...
    if outfitName then
        ...
        lib.callback("illenium-appearance:server:getOutfits", false, function(outfits)
            ...
            local converted = ConvertToCollectionFormat(cache.ped, pedProps, pedComponents)

            TriggerServerEvent("illenium-appearance:server:saveOutfit", outfitName, pedModel, converted.components, converted.props)
        end)
    end
end)
...
RegisterNetEvent('illenium-appearance:client:updateOutfit', function(outfitID)
    ...
    lib.callback("illenium-appearance:server:getOutfits", false, function(outfits)
        ...
        TriggerServerEvent("illenium-appearance:server:updateOutfit", outfitID, pedModel, converted.components, converted.props)
    end)
end)
...
RegisterNetEvent("illenium-appearance:client:SaveManagementOutfit", function(mType)
    local converted = ConvertToCollectionFormat(
        cache.ped,
...
RegisterNetEvent("illenium-appearance:client:changeOutfit", function(data)
    ...
    if appearanceDB then
        local migrateOutfit = data.id and not data.disableSave and not UsesCollectionFormat(data)
